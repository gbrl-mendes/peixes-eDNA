...
#carregando o arquivo de environment do projeto e Rmd e os pacotes necessarios
...
#definindo o caminho para o executavel do cutadapt
...
## Realizando a filtragem das reads com DADA2

Etapa prévia de filtragem das reads com o DADA2.

```{r, eval=FALSE,echo=TRUE}
# Filtrando as reads demultiplexadas ----

  # Será realizada a filtragem das reads brutas a partir dos seguintes parâmetros:
  
    # descarte das reads com N's (maxN = c(0,0))
    # remoção das reads com taxa de erro maior que 2 (maxEE = c(2,2))
    # remoção das reads não-pareadas (matchIDs = True)
    # descarte das reads de PhiX (isPhiX = True)

  # Outras flags:
  
    # O parâmetro 'compress = True' irá determinar que os arquivos de saída serão .gzip;
...
Agora que as reads estão limpas e repareadas, podemos iniciar o workflow do DADA2. 
A primeira etapa é aprender as taxas de erro específicas por read, que irão orientar
a dereplicação.
