...
## Versões paralelas ----
  ...
  ## O objeto cores_to_be_used define quantos núcleos do servidor serão utilizados para rodar o BLAST. Definiremos o número a partir do número total de núcleos fornecida pela função future::availableCores(), de onde subtrairemos o número de cores que não queremos que seja utilizado. 
cores_to_be_used <- future::availableCores() - 20 # Usar todos os cores -2 = 78 ## 
...
Cálculo da abundância das ASVs por amostra (RRA)
...
Esse trecho do código irá definir qual identificação das ASVs irá ser
incluída na tabela final de resultados. Se existe espécie e gênero
exatas, eles serão incluídos. Se não, apenas o gênero ou apenas a
espécie. Se essas identificações fornecidas pelo DADA2 não estiverem
disponíveis, então a atribuiição taxonômica será realizada com as
identificações feitas com as BLAST ids.
...
## Fatorização das colunas, para garantir que não existe hierarquia entre as identificações realizadas
all_ps_tbl_blast <- all_ps_tbl_blast %>%
mutate(
  Kingdom = factor(Kingdom),
  Phylum = factor(Phylum),
...
## Criação de .fasta para as ASVs

Esse trecho do código realiza a criação de um arquivo em formato FASTA
para armazenamento das ASVs recuperadas no pipeline. O arquivo final se
chama all_ASVs_all_primers.fasta e está armazenado nas pasta /results do
projeto.

```{r,echo=TRUE, eval=FALSE}

## Recuperar todas as sequências das ASVs para preparar o fasta ----

## Fornecendo nomes mais práticos para os cabeçalhos das sequências das ASVs (ASV_1, ASV_2...)
...
## Combinando os cabeçalhos das ASVs e o arquivo all_ps_tbl
all_ps_tbl_blast <- dplyr::left_join(x = all_ps_tbl_blast,
                                     y = all_asv_seqs,
                                     by = "ASV (Sequence)" )


## Criando e preenchendo o arquivo .fasta com as sequências das ASVs e suas respectivas identificações
for (asv in 1:nrow(all_asv_seqs)) {
  ...
  tax <- all_ps_tbl_blast %>%
    mutate(
      # Read_origin = unfactor(Read_origin),
      Family = unfactor(Family),
      Genus = unfactor(Genus),
      Species = unfactor(Species),
      `final ID` = unfactor(`final ID`)
    ) %>% 
    ungroup() %>% 
...
## Escrever o arquivo .fasta com as ASVs e os dados taxonômicos
all_asv_fasta <- c(rbind(all_asv_seqs$`ASV header`, all_asv_seqs$`ASV (Sequence)`))

write(all_asv_fasta, paste0(results_path,"/",prjct_rad,"-all_ASVs_all_primers.fasta")) ## essa é a saida do pipeline
...
Além da criação de ASVs, outro método de realizar a inferência das
sequências biológicas amplificadas é formação de *Operational Taxonomic
Units*, OTUs. O SWARM é um programa *open source* desenvolvido com o
objetivo de realizar a formação das OTUs superando os problemas dos
métodos clássicos: a dependência de um *threshold* global fixo e a
grande variabilidade na formação das OTUs formadas.
...
## Criação de um arquivo FASTA contendo as ASVs e suas respectivas informações taxonômicas
...
## all_ps_tbl_blast <- all_ps_tbl_blast %>% select(-c("ASV header.x","ASV length.x", "ASV header.y", "ASV length.y", "ASV header.x.x", "ASV length", "ASV header.y.y"))
